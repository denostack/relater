...
export class Relater<T extends RelateRules> {
  ...
  get size() {
    return (this.rules as RelateRule[]).reduce(
      (acc, rule) => acc + getSize(rule),
      0,
    );
  }
  ...
  relate(
    buffer: ArrayBuffer,
    { offset = 0 }: RelateOptions = {},
  ): RelatedObject<
    T
  > {
    return this.relateMany(buffer, { offset, limit: 1 })[0];
  }
  ...
  relateMany(
    ...
    for (const item of this.relateIterate(buffer, options)) {
      items.push(item);
    }
    return items;
  }
  ...
  *relateIterate(
    buffer: ArrayBuffer,
    { offset = 0, limit = Infinity }: RelateManyOptions = {},
  ): Generator<RelatedObject<T>> {
    const view = new DataView(buffer);
    let carryOffset = offset;
    const size = this.size;
    while (limit-- && carryOffset + size <= buffer.byteLength) {
      yield Object.create(
        {},
        Object.fromEntries(
            ...
            switch (rule.type) {
              ...
              case "i64": {
                return [rule.name, {
                  enumerable: true,
                  get: () =>
                    view.getBigInt64(offset, this.options.littleEndian),
                  set: (value: bigint) =>
                    view.setBigInt64(
                      offset,
                      value,
              ...
              case "u64": {
                return [rule.name, {
                  enumerable: true,
                  get: () =>
                    view.getBigUint64(offset, this.options.littleEndian),
                  set: (value: bigint) =>
                    view.setBigUint64(
                      offset,
                      value,
